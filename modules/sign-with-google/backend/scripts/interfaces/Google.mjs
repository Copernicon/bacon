...
export default class Google
{
	/**
		...
		@param {'login'|'register'|'add'} endpoint
	*/
	static async validateCode(code, endpoint)
	{
		try
		{
			const link =
			(
				...
				?	`https://${server.https.host}:${server.https.port}/sign-with-google/${endpoint}-google`
				:	`http://${server.http.host}:${server.http.port}/sign-with-google/${endpoint}-google`
			);
